...
- Слой данных: fetch с `AbortSignal`, валидация схемой, TanStack Query со stale time 5 с, нормализация в `OrgModel` внутри `queryFn`, агрегаты поддерева считаются здесь же один раз (нужны дереву).
- Дерево: раскрытие/скрытие, корни раскрыты по умолчанию, узел = название + суммарная численность + индикатор средней эффективности поддерева (собственные значения в подсказке), цветные направляющие вложенности; состояния загрузки / ошибки / пустого ответа.
