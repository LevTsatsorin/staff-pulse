...
- **Агрегаты уже на этапе 01.** Первый прогон показал, что собственные значения на родителе («дивизион — 5 чел.» над командой из 15) читаются как ошибка данных. Дерево показывает суммы поддерева, поэтому `aggregate.ts` (чистая функция, тесты) входит в `buildOrgModel` сразу; этап 02 только использует готовые агрегаты. → ADR-002.
- **Направляющие вложенности без пропа глубины:** цвет уровня задаётся CSS-переменными через селекторы вложенности `ul[role=group] ul[role=group] …` в `OrgTree`, `Group::before` рисует полосу и линию под колонкой шеврона родителя.
